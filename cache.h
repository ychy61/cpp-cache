...
private:
    struct Node {
        ...
        // int 타입 생성자
        Node(std::string k, int v) : key(k), type("int"), nextValue(nullptr), next(nullptr) {
            value = new int(v);
        }

        // double 타입 생성자
        Node(std::string k, double v) : key(k), type("double"), nextValue(nullptr), next(nullptr) {
            value = new double(v);
        }
    ...
    //노드를 앞쪽으로 이동시킨다.
    void addHead(Node* node, Node* prev, int index);
